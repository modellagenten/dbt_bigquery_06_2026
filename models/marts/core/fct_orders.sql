with
    stage_orders as (select * from {{ ref("stg_orders") }}),

    stage_order_items as (select * from {{ ref("stg_order_items") }}),

    stage_products as (select * from {{ ref("stg_products") }}),
